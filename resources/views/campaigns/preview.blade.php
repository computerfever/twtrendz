		  ...
		  <button class="tablinks" onclick="openTab(event, 'plain')">{{ trans('messages.web_view_plain_tab') }}</button>
			...
			<div class="ms-auto">
				<ul class="social-buttons">
					...
					<li class="button__share button__share--stumbleupon"><a href="{{action('Pub\CampaignController@previewContent', [ 'uid' => $campaign->uid, 'customer_uid' => Auth::user()->customer->uid ])}}" target="_blank">Public Url</a></li>

					<li class="button__share button__share--facebook">
						<a href="javascript:void(window.open('https://www.facebook.com/sharer.php?u=' + encodeURIComponent('{{action('Pub\CampaignController@previewContent', [ 'uid' => $campaign->uid, 'customer_uid' => Auth::user()->customer->uid ])}}') + '?t=' + encodeURIComponent(document.title),'_blank'))">
					...
					<li class="button__share button__share--twitter">
						<a href="javascript:void(window.open('https://twitter.com/share?url=' + encodeURIComponent('{{action('Pub\CampaignController@previewContent', [ 'uid' => $campaign->uid, 'customer_uid' => Auth::user()->customer->uid ])}}') + '&amp;text=' + encodeURIComponent(document.title),'_blank'))">
							<span class="d-flex align-items-center">
								<span class="me-2">
					...
					<li class="button__share button__share--linkedin">
						<a href="javascript:void(window.open('https://www.linkedin.com/shareArticle?url=' + encodeURIComponent('{{action('Pub\CampaignController@previewContent', [ 'uid' => $campaign->uid, 'customer_uid' => Auth::user()->customer->uid ])}}') + '&amp;title=' + encodeURIComponent(document.title),'_blank'))">
							<span class="d-flex align-items-center">
								<span class="me-2">
